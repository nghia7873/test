...
@section('breadcrumb')
    <ol class="breadcrumb" itemScope itemType="https://schema.org/BreadcrumbList">
        ...
        <li itemprop="itemListElement" itemscope="" itemtype="http://schema.org/ListItem">
            <a itemprop="item" href="{{ url()->current() }}" title="EP {{ $episode->name }}">
...
@section('content')
                ...
                <div class="row">
                    ...
                    <div class="col-3 movie-image">
                        <div class="movie-l-img">
                            <img itemprop="image" alt="{{ $currentMovie->name }}"
                                src="{{ $currentMovie->getThumbUrl() }}" />
                            <div class="movie-watch-link-box">
                                <a class="movie-watch-link"
                                    title="ดูภาพยนตร์ {{ $currentMovie->name }} ตอนที่ {{ $episode->name }}">
                                    {{ $episode->name }}</a>
                            </div>
                        </div>
                    </div>
        ...
        <div id="watch-block" class="small-player">
            ...
            <div id="go-server">
                <center>
                    <ul class="server-list">
                        <li class="backup-server"> <span class="server-title"> พากย์ไทย </span>
                            <ul class="list-episode">
                                <li class="episode">
                                    @foreach ($currentMovie->episodes->where('slug', $episode->slug)->where('server', $episode->server) as $server)
                                        <a data-id="{{ $server->id }}" data-link="{{ route('embed', ['id' => $server->link]) }}"
            ...
            <div class="user-action">
                <div id="btn-light" class="btn-cs light-off"><i class="btn-cs-icon icon-light-sm"></i><span
                        id="light-status">{{ __('auth.turn_off_light') }}</span></div>
                ...
                <div class="box-rating">
                    ...
                    <p>{{ __('auth.rate_movie')}} <span class="num-rating">({{$currentMovie->getRatingStar()}} คะแนน
    ...
    <div class="movie-list-index related-box">
        <h2 class="header-list-index">
            <span class="title-list-index">{{ __('auth.rate_movie')}}</span>
        </h2>
        <ul class="list-movie">
...
@push('scripts')
    ...
    <script>
        ...
        function chooseStreamingServer(el) {
            ...
            // TODO ROLLBACK
            const link = el.dataset.link.replace(/^http:\/\//i, 'http://');
